...
public class PlotRemoveOwnerCommand extends Command {

    private final InstanceGuard instanceGuard = API.getInstanceGuard();

    public PlotRemoveOwnerCommand() {
        super("removeowner");

        setDefaultExecutor((sender, context) -> sender.sendMessage("Usage: /plot removeowner <player>"));
        ...
        addSyntax((sender, context) -> {
            ...
            for (Region region : instanceGuard.getRegionManager().getRegions()) {
                if (region.containsLocation(player.getPosition())) {
                    ...
                    player.sendMessage(ChatUtils.translateMiniMessage("<dark_aqua>You have successfully removed <aqua>" + target.getUsername() + " <dark_aqua>as an owner of this plot."));
                } else {
                    player.sendMessage("<red>You are currently not on a plot.");
                }
            }
        }, playerArgument);
    }
